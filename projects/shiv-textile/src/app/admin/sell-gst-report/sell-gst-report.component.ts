...
@Component({
  selector: 'app-sell-gst-report',
  templateUrl: './sell-gst-report.component.html',
  styleUrls: ['./sell-gst-report.component.css']
})
export class SellGstReportComponent implements OnInit {
  employeeDetail: any;
  GstSellList: any[];
  ...
  //   /* save to file */
  //   XLSX.writeFile(wb, "Sell_GST.xlsx");
  // }
  ...
  exportToExcel() {
    ...
    var p = this.p;
    this.p = 1;
    this.itemPerPage = this.GstSellList.length;
    setTimeout(() => {
  ...
  getGstSellList() {
    ...
    this.service.getGstSellList(obj).subscribe(r1 => {
      let response = r1 as any;
      if (response.Message == ConstantData.SuccessMessage) {
        this.GstSellList = response.GstSellList;
        this.GstSellList.forEach((e1: any) => {
