...
@Component({
  selector: 'app-merge-return-list',
  templateUrl: './merge-return-list.component.html',
  styleUrls: ['./merge-return-list.component.css']
})
export class MergeReturnListComponent implements OnInit {
  employeeDetail: any;
  MergeReturnList: any[];
  ...
  exportToExcel() {
    ...
    var p = this.p;
    this.p = 1;
    this.itemPerPage = this.MergeReturnList.length;
    setTimeout(() => {
      this.loadDataService.exportToExcel(this.table, "Return GST Report " + this.loadDataService.loadDateTime(new Date()));
  ...
  getMergeReturnList() {
    ...
    this.service.getMergeReturnList(obj).subscribe(r1 => {
      let response = r1 as any;
      if (response.Message == ConstantData.SuccessMessage) {
        this.MergeReturnList = response.MergeReturnList;
        this.MergeReturnList.forEach((e1: any) => {
