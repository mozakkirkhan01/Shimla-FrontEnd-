...
@Component({
  selector: 'app-direct-sell-gst-report',
  templateUrl: './direct-sell-gst-report.component.html',
  styleUrls: ['./direct-sell-gst-report.component.css']
})
export class DirectSellGstReportComponent implements OnInit {
  employeeDetail: any;
  GstDirectSellList: any[];
  ...
  exportToExcel() {
    ...
    var p = this.p;
    this.p = 1;
    this.itemPerPage = this.GstDirectSellList.length;
    setTimeout(() => {
      this.loadDataService.exportToExcel(this.table, "Direct Sell GST Report " + this.loadDataService.loadDateTime(new Date()));
  ...
  getGstDirectSellList() {
    ...
    this.service.getGstDirectSellList(obj).subscribe(r1 => {
      let response = r1 as any;
      if (response.Message == ConstantData.SuccessMessage) {
        this.GstDirectSellList = response.GstDirectSellList;
        this.GstDirectSellList.forEach((e1: any) => {
