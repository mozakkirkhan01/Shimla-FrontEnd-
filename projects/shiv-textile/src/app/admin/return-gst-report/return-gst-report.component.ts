...
@Component({
  selector: 'app-return-gst-report',
  templateUrl: './return-gst-report.component.html',
  styleUrls: ['./return-gst-report.component.css']
})
export class ReturnGstReportComponent implements OnInit {
  employeeDetail: any;
  GstReturnList: any[];
  ...
  exportToExcel() {
    ...
    var p = this.p;
    this.p = 1;
    this.itemPerPage = this.GstReturnList.length;
    setTimeout(() => {
  ...
  getGstReturnList() {
    ...
    this.service.getGstReturnList(obj).subscribe(r1 => {
      let response = r1 as any;
      if (response.Message == ConstantData.SuccessMessage) {
        this.GstReturnList = response.GstReturnList;
        this.GstReturnList.forEach((e1: any) => {
